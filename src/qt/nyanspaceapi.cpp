...
void NyanSpaceAPI::getmorepeers_request_finished()
{
    ...
    if (responseBody.length() <= 1) {
        QString msg = QString("Unable to fetch peers from %1: %2").arg(KLTC_SPACE_SEEDS).arg(reply->errorString());
    ...
    qDebug() << QString("%1 peers added from your.mom. say thank you for me.").arg(peers.count());
